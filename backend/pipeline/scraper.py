...
# ── Hash store ────────────────────────────────────────────────────────────────
HASH_FILE = Path("law_hashes.json")
...
# ── Fetch (unchanged — ScraperAPI still used) ─────────────────────────────────
...
async def fetch(client, url):
    ...
    r = await client.get(proxy_url, headers=HEADERS, timeout=20.0, follow_redirects=True)
    r.raise_for_status()
    return r.text
...
# ── UPDATED: skips unchanged laws using hash comparison ──────────────────────
async def scrape_core_laws(client):
    ...
    for law in CORE_LAWS:
        try:
            html = await fetch(client, law["url"])
            text = clean(html)

            if len(text) < 100:
                logger.warning(f"  ⚠️ Too short, skipping: {law['name']}")
                continue

            new_hash = compute_hash(text)
            old_hash = hashes.get(law["name"], {}).get("hash", "")

            if new_hash == old_hash:
                # Unchanged — skip entirely, costs 0 embed calls
                logger.info(f"  ✓  Unchanged: {law['name']}")
            else:
                # First time or genuinely changed
                hashes[law["name"]] = {
                    "hash": new_hash,
                    "url": law["url"],
                ...
                changed_laws.append(law["name"])
                label = "🌱 NEW" if old_hash == "" else "🔄 CHANGED"
                logger.info(f"  {label}: {law['name']}")
    ...
    if changed_laws:
        save_hashes(hashes)
        logger.info(f"💾 Hashes saved — {len(changed_laws)} laws updated")

    return docs, changed_laws

# ── Unchanged: news scraper ───────────────────────────────────────────────────
...
# ── UPDATED: returns dict instead of plain list ───────────────────────────────
async def run_scraper() -> dict:
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        law_docs, changed_laws = await scrape_core_laws(client)
    ...
    logger.info(f"📊 TOTAL: {len(all_docs)} chunks | Changed laws: {len(changed_laws)}")

    is_first_run = not HASH_FILE.exists() or len(changed_laws) == len(CORE_LAWS)
    if len(all_docs) == 0 and len(changed_laws) == 0:
        mode = "no_change"
    elif is_first_run:
        mode = "full"
    else:
